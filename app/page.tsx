...
const bubbleColors = ["#15803d", "#f59e0b", "#3b82f6", "#ef4444", "#8b5cf6", "#06b6d4"]
...
export default function QuestionBoard() {
  const [questions, setQuestions] = useState<Question[]>([
    {
      ...
      radius: Math.max(48, Math.min(120, (60 + "웹소켓에 대해 설명해주실 분 계신가요?".length * 3) * 0.64)),
      mass: Math.max(1, "웹소켓에 대해 설명해주실 분 계신가요?".length * 0.1),
      comments: [
        {
          id: 1,
    ...
    },
    {
      ...
      vx: (Math.random() - 0.5) * 1,
      vy: (Math.random() - 0.5) * 1,
      radius: Math.max(48, Math.min(120, (60 + "React 18의 새로운 기능들이 궁금해요!".length * 3) * 0.64)),
      mass: Math.max(1, "React 18의 새로운 기능들이 궁금해요!".length * 0.1),
      comments: [],
    },
    {
      id: 3,
      ...
      vx: (Math.random() - 0.5) * 1,
      vy: (Math.random() - 0.5) * 1,
      radius: Math.max(48, Math.min(120, (60 + "TypeScript 타입 가드 사용법?".length * 3) * 0.64)),
    ...
    },
    {
      ...
      vx: (Math.random() - 0.5) * 1,
      vy: (Math.random() - 0.5) * 1,
      radius: Math.max(48, Math.min(120, (60 + "Next.js 13 App Router 어떻게 사용하나요?".length * 3) * 0.64)),
      mass: Math.max(1, "Next.js 13 App Router 어떻게 사용하나요?".length * 0.1),
      comments: [],
    },
    {
      id: 5,
      ...
      vx: (Math.random() - 0.5) * 1,
      vy: (Math.random() - 0.5) * 1,
      radius: Math.max(48, Math.min(120, (60 + "CSS Grid vs Flexbox 언제 뭘 써야 할까요?".length * 3) * 0.64)),
      mass: Math.max(1, "CSS Grid vs Flexbox 언제 뭘 써야 할까요?".length * 0.1),
      comments: [],
    },
  ])
  ...
  const p5InstanceRef = useRef<P5CanvasInstance | null>(null)
    ...
    const loadP5 = async () => {
      ...
      const sketch = (p: P5CanvasInstance) => {
        ...
        p.draw = () => {
          ...
          const currentQuestions = questionsRef.current.map((question) => {
            const newQuestion = { ...question }

            const mouseDistance = Math.sqrt((p.mouseX - newQuestion.x) ** 2 + (p.mouseY - newQuestion.y) ** 2)
            const isHovered = mouseDistance < newQuestion.radius
            ...
            if (newQuestion.y + newQuestion.radius > p.height - 100) {
              newQuestion.y = p.height - 100 - newQuestion.radius
              newQuestion.vy *= -bounce
            }
          ...
          for (let i = 0; i < currentQuestions.length; i++) {
            for (let j = i + 1; j < currentQuestions.length; j++) {
              ...
              if (distance < minDistance) {
                ...
                const targetX = q1.x + Math.cos(angle) * minDistance
                const targetY = q1.y + Math.sin(angle) * minDistance
          ...
          currentQuestions.forEach((question) => {
            const mouseDistance = Math.sqrt((p.mouseX - question.x) ** 2 + (p.mouseY - question.y) ** 2)
            const isHovered = mouseDistance < question.radius
            const hoverScale = isHovered ? 1.05 : 1

            p.fill(0, 0, 0, 20)
            p.noStroke()
            p.ellipse(question.x + 2, question.y + 4, question.radius * 2, question.radius * 2.2)
            ...
            const stringStartY = question.y + question.radius * 1.1 * hoverScale
            p.line(question.x, stringStartY, question.x + stringWave, stringStartY + 50)
            ...
            p.strokeWeight(2)
            p.ellipse(question.x, question.y, question.radius * 2 * hoverScale, question.radius * 2.2 * hoverScale)
            ...
            for (let i = 0; i < title.length; i++) {
              const char = title[i]
              const testLine = currentLine === 1 ? line1 + char : line2 + char
            ...
            const lineHeight = 18
            if (line2.trim()) {
              p.text(line1.trim(), question.x, question.y - lineHeight / 2)
              p.text(line2.trim(), question.x, question.y + lineHeight / 2)
            } else {
        ...
        p.mousePressed = () => {
          ...
          questionsRef.current.forEach((question) => {
            const distance = Math.sqrt((p.mouseX - question.x) ** 2 + (p.mouseY - question.y) ** 2)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (newQuestion.trim()) {
      ...
      const radius = Math.max(48, Math.min(120, (60 + limitedTitle.length * 3) * 0.64))
  ...
  const handleAddComment = (e: React.FormEvent) => {
    ...
    if (newComment.trim() && selectedQuestion) {
      ...
      setQuestions((prev) =>
        prev.map((q) => (q.id === selectedQuestion.id ? { ...q, comments: [...q.comments, comment] } : q)),
      )
    ...
    <div className="min-h-screen bg-slate-50 overflow-hidden">
      ...
      {selectedQuestion && (
        ...
        >
          ...
          >
            <div className="flex-1 p-8 overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-start mb-6">
                <div className="flex-1">
                  <div className="w-8 h-8 rounded-full mb-4" style={{ backgroundColor: selectedQuestion.color }} />
                  <p className="text-sm text-gray-500 mb-2">{selectedQuestion.timestamp.toLocaleString("ko-KR")}</p>
                </div>
                <Button
                  variant="ghost"
                  size="icon"
              ...
              <div className="space-y-4" onClick={(e) => e.stopPropagation()}>
                {(() => {
                  ...
                  return (
                    <>
                      <h2 className="text-2xl font-bold text-gray-900 leading-relaxed">{title}</h2>

                      {body && (
                        <div className="bg-gray-50 rounded-2xl p-6" onClick={(e) => e.stopPropagation()}>
                          <p className="text-gray-700 leading-relaxed whitespace-pre-wrap">{body}</p>
                        </div>
                      )}
                      ...
                      {!body && (
                        <div className="bg-gray-50 rounded-2xl p-6" onClick={(e) => e.stopPropagation()}>
                          <p className="text-gray-600 leading-relaxed">
                            이 질문에 대한 답변을 기다리고 있어요! 오른쪽에서 댓글로 답변해주세요. 🎈
                          </p>
                        </div>
                      )}
                    </>
            ...
            <div className="w-96 border-l border-gray-200 flex flex-col" onClick={(e) => e.stopPropagation()}>
              <div className="p-6 border-b border-gray-200" onClick={(e) => e.stopPropagation()}>
                <h3 className="text-lg font-semibold text-gray-900">댓글 ({selectedQuestion.comments.length})</h3>
              </div>

              <div className="flex-1 overflow-y-auto p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
                {selectedQuestion.comments.length === 0 ? (
                  <p className="text-gray-500 text-center py-8" onClick={(e) => e.stopPropagation()}>
                    아직 댓글이 없어요.
                    <br />첫 번째 댓글을 남겨보세요! 💬
                  </p>
                ) : (
                  selectedQuestion.comments.map((comment) => (
                    <div key={comment.id} className="bg-gray-50 rounded-2xl p-4" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center gap-2 mb-2">
                        <div className="w-6 h-6 rounded-full" style={{ backgroundColor: selectedQuestion.color }} />
                        <span className="text-sm font-medium text-gray-700">{comment.author}</span>
                        <span className="text-xs text-gray-500">{comment.timestamp.toLocaleString("ko-KR")}</span>
                      </div>
              ...
              <div className="p-6 border-t border-gray-200" onClick={(e) => e.stopPropagation()}>
                <form onSubmit={handleAddComment} className="space-y-3">
                  <Input
                    ...
                    onClick={(e) => e.stopPropagation()}
                  />
                  <Button type="submit" className="w-full" style={{ backgroundColor: selectedQuestion.color }}>
